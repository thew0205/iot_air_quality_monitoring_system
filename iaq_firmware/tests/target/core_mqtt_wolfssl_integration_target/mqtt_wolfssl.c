...
int EthernetSend(WOLFSSL *ssl, char *message, int sz, void *ctx)
{
    ...
    if (err != ERR_OK)
    {
        if (err == ERR_CONN || err == ERR_RST || err == ERR_CLSD || err == ERR_ABRT || err == ERR_RTE || err == ERR_IF)
        {
...
int EthernetReceive(WOLFSSL *ssl, char *reply, int sz, void *ctx)
{
    ...
    while (sz != 0)
    {
        ...
        else if (current_available_data == 0 && received == 0)
        {
            LOGW(TAG, "No data available to read, retrying...\n");
